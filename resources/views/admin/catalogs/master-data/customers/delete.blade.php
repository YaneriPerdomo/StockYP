        ...
        <article class=" h-100 w-adjustable flex-full__aligh-start  ">
            <div class="card  p-3">
                ...
                <div class="card__message card__message--delete-acount">
                    ...
                    <p>
                        La eliminación de @if ($gender_id == 1)
                            este cliente
                        @else
                            esta clienta
                        @endif es una acción irreversible. Al proceder, se eliminarán
                        definitivamente todos los datos de ventas,
                        el historial completo y la información relacionada. Ten en cuenta que, una vez borrado, este
                        registro no se podrá restaurar ni
                        aparecerá en futuros informes.
                    </p>
                </div>
                <hr>
                <div class="card__button-option flex-full__justify-content-between ">
                    <a href="{{ route('customer.index') }}" class="text-decoration-white card__link-cancel">
                        <button class="button button--color-grey" type="submit">
                            Cancelar
                        </button>
                    </a>
                    <form action="{{ route('customer.destroy', $slug) }}" method="POST">
                        @method('DELETE')
                        @csrf
