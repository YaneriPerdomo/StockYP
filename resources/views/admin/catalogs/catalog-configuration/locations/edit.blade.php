...
<head>
    ...
    <title>Editar ubicación | <x-systen-name></x-systen-name></title>
...
<body class="h-100 d-flex flex-column">
    ...
    <main class="flex__grow-2 flex-full__aligh-start">
        <form action="{{ route('location.update', $location->slug) }}" method="post" class="form  w-adjustable-s">
            @csrf
            @method('PUT')
            <div class="button--back">
                <a href="{{ route('location.index') }}">
                    <i class="bi bi-arrow-left-square text-grey"></i> <button class="button text-grey"
                        type="button">Volver al listado de ubicaciones</button>
                </a>
            </div>
            <legend class="form__title">
                <b> Editar ubicación</b>
            </legend>
            ...
            <div class="form__item">
                <label for="" class="form__label form__label--required">Detalles de ubicación</label>
                <div class="input-group ">
                    <span class="form__icon input-group-text @error ('name') is-invalid--border @enderror"
                        id="basic-addon1"><i class="bi bi-pin-map"></i></span>
